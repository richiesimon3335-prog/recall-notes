...
type NoteAny = {
  ...
  created_at?: string;
  // 下面两个字段你当前 schema 里可能没有，所以做成可选
  topics?: string[] | null;
  keywords?: string[] | null;
};

function safeJoin(v: unknown): string {
  if (!v) return "";
...
export default async function NoteDetailPage({
  ...
  // ✅ 用一个更宽松的类型承接，避免因为 schema 差异导致 TS build 崩
  const n = res.data as NoteAny;
  ...
  // ✅ topics / keywords 做安全读取（你现在没有也不会报错）
  const topicsText = safeJoin((n as any)?.topics);
  const keywordsText = safeJoin((n as any)?.keywords);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="space-y-3">
        ...
        {/* ✅ topics/keywords：存在则展示，不存在就不显示（避免 TS build 崩） */}
        {(topicsText || keywordsText) ? (
          <div className="pt-2 text-xs text-zinc-600">
            {topicsText ? (
              <>
                Related ideas: {topicsText}
                <br />
              </>
            ) : null}
            {keywordsText ? <>Key themes: {keywordsText}</> : null}
          </div>
        ) : null}
      </Card>
